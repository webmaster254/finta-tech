...
class LoanDisbursementChart extends ApexChartWidget
{
   ...
   protected function getOptions(): array
   {
       ...
       match ($timeFilters) {
           ...
           'week' => $data = Trend::query(Loan::where('loan_officer_id', auth()->user()->id ?? ''))
           ...
           'month' => $data = Trend::query(Loan::where('loan_officer_id', auth()->user()->id ?? ''))
                   ->dateColumn('disbursed_on_date')
                       ->between(
                           start: now()->subMonth(),
                           end: now(),
                       ...
                       ->perDay()
                       ->sum('principal_disbursed_derived'),
           '3months' => $data = Trend::query(Loan::where('loan_officer_id', auth()->user()->id ?? ''))
                   ->dateColumn('disbursed_on_date')
                       ->between(
                       ...
                       ->perMonth()
                       ->sum('principal_disbursed_derived'),
           'year' => $data = Trend::query(Loan::where('loan_officer_id', auth()->user()->id ?? ''))
                   ->dateColumn('disbursed_on_date')
                       ->between(
